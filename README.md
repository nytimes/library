...
Documentation about how to get started with Library is hosted as a working (read only) demo on Heroku. Consult the site for more detailed instructions than this readme about how to get the most out of Library: https://library-demo.newsdev.nytimes.com/.
...
See [CONTRIBUTING.md](https://github.com/nytimes/library/blob/master/CONTRIBUTING.md) for information on how to contribute code and/or documentation on GitHub or on the [demo site](https://library-demo.newsdev.nytimes.com/).
...
The app should now be running at `localhost:3000`. Library requires the Node version specified in [`.nvmrc`](.nvmrc); `package.json` sets a floor of Node 18. If you use a version manager such as `nvm` or `fnm`, running `nvm use` or `fnm use` in the project root will select the correct version.

### Tests

Run `npm run build` before running the tests for the first time, or after changing anything under `styles/`. The error-page tests assert that styles are inlined, which requires the compiled `public/css/errors.css` produced by `npm run build:inline`. Without it, `test/functional/pages.test.js` fails on a fresh clone.
...
In addition to the variables in the example `.env` above, Library reads the following optional environment variables. Defaults are the values used when the variable is unset.

| Variable                         | Default                                         | Description                                                                                        |
| -------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `PORT`                           | `3000`                                          | Port the server listens on.                                                                        |
| `TRUST_PROXY`                    | unset                                           | Set to `true` (case-insensitive) to trust the `X-Forwarded-*` headers from an upstream proxy.       |
| `PATH_PREFIX`                    | `/`                                             | Path prefix the site is served under.                                                              |
| `DRIVE_TIMEOUT_SECONDS`          | `60`                                            | Timeout for Drive API requests, in seconds.                                                        |
| `LIST_UPDATE_DELAY`              | `15`                                            | Delay between drive tree refreshes, in seconds.                                                    |
| `EDIT_CACHE_DELAY`               | `3600`                                          | How long an edited document is kept out of the cache, in seconds.                                  |
| `ALLOW_INLINE_CODE`              | `false`                                         | Set to `true` to allow inline HTML from documents to render.                                       |
| `REDIRECT_URL`                   | `/auth/redirect`, prefixed by `PATH_PREFIX`     | oAuth callback URL.                                                                                |
| `TEST_EMAIL`                     | `footer.defaultEmail` from the string templates | Email for the mock user in development. Only read when `NODE_ENV=development`.                     |
| `GOOGLE_APPLICATION_CREDENTIALS` | `server/.auth.json` in development              | Path to the service account credentials file.                                                      |
| `LOG_LEVEL`                      | `debug` in development, otherwise `info`        | Winston log level. Currently not honored; see [#379](https://github.com/nytimes/library/pull/379). |
...
For more detailed instructions, consult the Getting Started section of the demo site: https://library-demo.newsdev.nytimes.com//get-started
...
Set your app's `GOOGLE_APPLICATION_JSON`, `GOOGLE_CLIENT_ID`, and `GOOGLE_CLIENT_SECRET` with values from the service account and Oauth client. Add _<your-heroku-app-url\>.com_ as an authorized domain in the general OAuth consent screen setup and then add _http://<your-heroku-app-url\>.com/auth/redirect_ as the callback url in the OAuth credential setup itself.

### Using Google App Engine

You can also deploy Library to GAE, using the included `app.yaml`. Note that you will need to enable billing on your GCP project in order to use Google App Engine. More detailed instructions are provided on the [demo site](https://library-demo.newsdev.nytimes.com//get-started/deploying-library-google-app-engine).
...
The tree and file metadata are repopulated into memory on an interval (15s by default, configurable via `LIST_UPDATE_DELAY`). Calling getTree multiple times will not return fresher data.
